...
const querySchema = z.object({ page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(1).max(100).default(100), store: z.string().trim().max(100).optional(), keyword: z.string().trim().max(200).optional(), cursor: z.string().trim().max(200).optional() })

export const pricingRoutes = new Hono<AuthEnv>()
pricingRoutes.get('/', requireAuth, requirePermission('pricing.read'), async (context) => {
  ...
  return context.json(await listPricing({ ...query, studioId: context.get('authContext').currentStudio.id }))
})

pricingRoutes.post('/import', requireAuth, requirePermission('pricing.import'), async (context) => {
  ...
  // 防御性校验：单次导入行数限制在 0～50000
  if (items.length > 50000) return context.json({ code: 'VALIDATION_ERROR', message: '单次导入行数不能超过 50000。' }, 400)
  const batch = await createPricingImport({ studioId: context.get('authContext').currentStudio.id, fileName: file.name || '未命名文件.xlsx', uploadedBy: context.get('authUser').id, items })
